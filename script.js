...
// NEW: Global variable to store episode data for the currently open series
let currentSeriesEpisodes = {};
let currentSeriesId = null;
let currentSeriesTitle = '';
let currentSeriesSectionType = ''; // NEW: To store the section type for episode context
...
const sectionTypesConfig = { // Matching admin panel
    'talk_shows': { displayName: 'Talk Shows & Reality', cardType: 'content-card', supportsEpisodes: true },
    'kdrama_anime': { displayName: 'K-drama & Anime', cardType: 'content-card', supportsEpisodes: true },
    'maqbool_series': { displayName: 'Maqbool Series', cardType: 'content-card', supportsEpisodes: true },
    'finance_crypto': { displayName: 'Finance, Market & Crypto', cardType: 'content-card', supportsEpisodes: true },
    'live_sports': { displayName: 'Live Sports', cardType: 'wide-card', supportsEpisodes: true },
    'news_entertainment': { displayName: 'News & Entertainment', cardType: 'content-card', supportsEpisodes: true },
    'dramas': { displayName: 'Top 10 Popular Movies & Songs (Drama Swiper)', cardType: 'drama-card', supportsEpisodes: false }
};
 ...
 * @param {string} actionType - The type of action to perform ('playGameUrl', 'joinTournament', 'openBannerLink', 'openYouTubeBannerVideo', 'openDramaVideoPlayer', 'openEpisodeSelectionModal').
...
function checkLoginAndAct(event, actionType, ...args) {
    ...
    } else if (actionType === 'openDramaVideoPlayer') { 
        openDramaVideoPlayer(...args);
    } else if (actionType === 'openEpisodeSelectionModal') { // NEW: Handle episode selection action
        openEpisodeSelectionModal(...args);
    }
}
...
auth.onAuthStateChanged(async user => {
    ...
    if (user) {
        ...
        try {
            ...
            // Handle pending action after login (e.g., join tournament, play game, open banner link, open YouTube banner video, open drama video player, openEpisodeSelectionModal)
            if (pendingAction) {
                ...
                else if (type === 'openDramaVideoPlayer') openDramaVideoPlayer(...args);
                else if (type === 'openEpisodeSelectionModal') openEpisodeSelectionModal(...args); // NEW: Handle pending episode selection
            }
...
function escapeJsStringForHtmlAttribute(s) {
    ...
    return str.replace(/\\/g, '\\\\')
              .replace(/'/g, '&#39;') // Use HTML entity for single quotes
              .replace(/"/g, '&quot;')
              .replace(/\n/g, '\\n')
              .replace(/\r/g, '\\r');
}
...
function displaySearchResults(loadMore = false) {
    ...
    async function renderHomePage(container) {
        ...
        // NEW: Clear and set loading states for custom sections
        document.getElementById('talk-shows-container').innerHTML = `<div class="w-full text-center py-8 text-gray-400 italic">Loading Talk Shows...</div>`;
        document.getElementById('kdrama-anime-container').innerHTML = `<div class="w-full text-center py-8 text-gray-400 italic">Loading K-dramas & Anime...</div>`;
        document.getElementById('maqbool-series-container').innerHTML = `<div class="w-full text-center py-8 text-gray-400 italic">Loading Maqbool Series...</div>`;
        document.getElementById('finance-crypto-container').innerHTML = `<div class="w-full text-center py-8 text-gray-400 italic">Loading Finance & Crypto...</div>`;
        document.getElementById('live-sports-container').innerHTML = `<div class="w-full text-center py-8 text-gray-400 italic">Loading Live Sports...</div>`;
        document.getElementById('news-entertainment-container').innerHTML = `<div class="w-full text-center py-8 text-gray-400 italic">Loading News & Entertainment...</div>`;
        ...
        // --- NEW SECTIONS: Dynamic Content Loading ---

        // Helper function to render content cards
        const renderContentCards = (containerId, dataArray, sectionType) => {
            const containerEl = document.getElementById(containerId);
            if (!containerEl) return;

            if (dataArray.length === 0) {
                containerEl.innerHTML = `<div class="w-full text-center py-8 text-gray-400 italic">No content available.</div>`;
                return;
            }

            containerEl.innerHTML = dataArray.map(item => {
                const config = sectionTypesConfig[sectionType];
                // If the section supports episodes, clicking the card opens the episode selection modal
                const onclickAction = config.supportsEpisodes
                    ? `checkLoginAndAct(event, 'openEpisodeSelectionModal', '${escapeJsStringForHtmlAttribute(item.id)}', '${escapeJsStringForHtmlAttribute(item.title)}', '${sectionType}')`
                    : `checkLoginAndAct(event, 'openBannerLink', '${escapeJsStringForHtmlAttribute(item.link_url)}')`; // Fallback for simple link items

                return `
                    <div class="content-card" onclick="${onclickAction}">
                        <div class="thumb">
                            <img src="${escapeJsStringForHtmlAttribute(item.image_url || 'https://via.placeholder.com/140x200?text=Image')}" alt="${escapeJsStringForHtmlAttribute(item.title)}" loading="lazy">
                            ${item.badge ? `<span class="badge ${item.badge.toLowerCase()}">${escapeJsStringForHtmlAttribute(item.badge)}</span>` : ''}
                            ${item.badge2 ? `<span class="badge2">${escapeJsStringForHtmlAttribute(item.badge2)}</span>` : ''}
                        </div>
                        <div class="title">${escapeJsStringForHtmlAttribute(item.title)}</div>
                        <div class="meta">${escapeJsStringForHtmlAttribute(item.meta || '')}</div>
                    </div>
                `
            }).join('');
        };

        // Helper function to render wide cards (for Live Sports)
        const renderWideCards = (containerId, dataArray, sectionType) => {
            const containerEl = document.getElementById(containerId);
            if (!containerEl) return;

            if (dataArray.length === 0) {
                containerEl.innerHTML = `<div class="w-full text-center py-8 text-gray-400 italic">No live sports content available.</div>`;
                return;
            }
            ...
            containerEl.innerHTML = dataArray.map(item => {
                ...
                const onclickAction = config.supportsEpisodes
                    ? `checkLoginAndAct(event, 'openEpisodeSelectionModal', '${escapeJsStringForHtmlAttribute(item.id)}', '${escapeJsStringForHtmlAttribute(item.title)}', '${sectionType}')`
                    : `checkLoginAndAct(event, 'openBannerLink', '${escapeJsStringForHtmlAttribute(item.link_url)}')`;

                return `
                    <div class="wide-card" onclick="${onclickAction}">
                        <div class="thumb">
                            <img src="${escapeJsStringForHtmlAttribute(item.image_url || 'https://via.placeholder.com/260x146?text=Live+Sport')}" alt="${escapeJsStringForHtmlAttribute(item.title)}" loading="lazy">
                            <div class="overlay">
                                ${item.livedot ? `<div class="tag"><span class="livedot"></span> LIVE</div>` : ''}
                                <h3>${escapeJsStringForHtmlAttribute(item.title)}</h3>
                                <p>${escapeJsStringForHtmlAttribute(item.description || '')}</p>
                            </div>
                        </div>
                    </div>
                `
        ...
        // Load Talk Shows aur Reality
        db.ref('talk_shows').on('value', snapshot => {
            const items = [];
            snapshot.forEach(child => {
                const item = child.val();
                if (item.status === 'active') items.push({ id: child.key, ...item });
            });
            items.sort((a,b) => (a.order || 0) - (b.order || 0));
            renderContentCards('talk-shows-container', items, 'talk_shows');
        });

        // Load K-drama aur anime
        db.ref('kdrama_anime').on('value', snapshot => {
            const items = [];
            snapshot.forEach(child => {
                const item = child.val();
                if (item.status === 'active') items.push({ id: child.key, ...item });
            ...
            items.sort((a,b) => (a.order || 0) - (b.order || 0));
            renderContentCards('kdrama-anime-container', items, 'kdrama_anime');
        });

        // Load Maqbool Series
        db.ref('maqbool_series').on('value', snapshot => {
            const items = [];
            snapshot.forEach(child => {
                const item = child.val();
                if (item.status === 'active') items.push({ id: child.key, ...item });
            ...
            items.sort((a,b) => (a.order || 0) - (b.order || 0));
            renderContentCards('maqbool-series-container', items, 'maqbool_series');
        });

        // Load Finance, Market & Crypto
        db.ref('finance_crypto').on('value', snapshot => {
            const items = [];
            snapshot.forEach(child => {
                const item = child.val();
                if (item.status === 'active') items.push({ id: child.key, ...item });
            ...
            items.sort((a,b) => (a.order || 0) - (b.order || 0));
            renderContentCards('finance-crypto-container', items, 'finance_crypto');
        });

        // Load Live Sports
        db.ref('live_sports').on('value', snapshot => {
            const items = [];
            snapshot.forEach(child => {
                const item = child.val();
                if (item.status === 'active') items.push({ id: child.key, ...item });
            ...
            items.sort((a,b) => (a.order || 0) - (b.order || 0));
            renderWideCards('live-sports-container', items, 'live_sports');
        });

        // Load News & Entertainment (Original section, now dynamic)
        db.ref('news_entertainment').on('value', snapshot => {
            const items = [];
            snapshot.forEach(child => {
                const item = child.val();
                if (item.status === 'active') items.push({ id: child.key, ...item });
            ...
            items.sort((a,b) => (a.order || 0) - (b.order || 0));
            renderContentCards('news-entertainment-container', items, 'news_entertainment');
        });
        // --- END NEW SECTIONS ---
    }
    ...
    /**
     * NEW: Opens the episode selection modal for a given series.
     * @param {string} seriesId - The ID of the series (e.g., from talk_shows, kdrama_anime, etc.).
     * @param {string} seriesTitle - The title of the series.
     * @param {string} sectionType - The Firebase section type (e.g., 'talk_shows').
     */
    async function openEpisodeSelectionModal(seriesId, seriesTitle, sectionType) {
        if (!auth.currentUser) {
            showToast('Login required to view episodes!', true);
            return;
        }
        ...
        currentSeriesSectionType = sectionType; // Store section type
        document.getElementById('episode-modal-title').textContent = seriesTitle;
        document.getElementById('episode-video-player-container').innerHTML = `
            <div id="video-placeholder-content" class="w-full h-full flex flex-col items-center justify-center">
                <i class="fab fa-youtube fa-5x text-red-600 mb-4"></i>
                <p class="text-lg text-gray-300">Select an episode to play</p>
            </div>
        `;
        document.getElementById('episode-buttons-grid').innerHTML = '<p class="text-center text-gray-500 italic col-span-3 py-4">Loading episodes...</p>';

        toggleModal('episodeSelectionModal', true);

        try {
            const snapshot = await db.ref(`${sectionType}/${seriesId}/episodes`).once('value');
            const episodesData = snapshot.val();
            currentSeriesEpisodes = {};
            if (episodesData) {
                for (const id in episodesData) {
                    currentSeriesEpisodes[id] = { id, ...episodesData[id] };
                }
            }
            renderEpisodeButtons();
        } catch (error) {
            console.error("Error fetching episodes:", error);
            document.getElementById('episode-buttons-grid').innerHTML = '<p class="text-center text-red-500 italic col-span-3 py-4">Error loading episodes.</p>';
            showToast("Failed to load episodes. Please try again.", true);
        }
    }

    /**
     * NEW: Renders the episode buttons in the modal.
     */
    async function renderEpisodeButtons() {
        const gridEl = document.getElementById('episode-buttons-grid');
        if (!gridEl) return;

        if (!currentSeriesEpisodes || Object.keys(currentSeriesEpisodes).length === 0) {
            gridEl.innerHTML = '<p class="text-center text-gray-500 italic col-span-3 py-4">No episodes available for this series.</p>';
            return;
        }

        let buttonsHtml = '';
        const unlockedEpisodeTimestamps = currentUserData?.unlocked_episodes || {};

        const sortedEpisodeKeys = Object.keys(currentSeriesEpisodes).sort((a, b) => {
            const epNumA = currentSeriesEpisodes[a].order || 0;
            const epNumB = currentSeriesEpisodes[b].order || 0;
            return epNumA - epNumB;
        });

        for (const epKey of sortedEpisodeKeys) {
            const episode = currentSeriesEpisodes[epKey];
            
            // Check if it's permanently unlocked (admin set is_locked: false)
            const isPermanentlyUnlocked = !episode.is_locked;
            
            // Check for temporary 24-hour unlock (from user's profile)
            const unlockTimestamp = unlockedEpisodeTimestamps[episode.id];
            const isTemporarilyUnlocked = unlockTimestamp && (Date.now() < new Date(unlockTimestamp).getTime());

            const isUnlocked = isPermanentlyUnlocked || isTemporarilyUnlocked;

            let buttonClass, buttonIcon, onclickAction;

            if (isUnlocked) {
                buttonClass = 'episode-button bg-gray-700 hover:bg-red-600 text-white';
                buttonIcon = '';
                onclickAction = `playEpisode('${escapeJsStringForHtmlAttribute(episode.id)}', '${escapeJsStringForHtmlAttribute(episode.video_url)}')`;
            } else {
                buttonClass = 'episode-button bg-gray-800 text-gray-500 cursor-pointer hover:bg-gray-700';
                buttonIcon = '<i class="fas fa-lock ml-2 text-gray-500"></i>';
                onclickAction = `unlockEpisodePrompt('${escapeJsStringForHtmlAttribute(episode.id)}', '${escapeJsStringForHtmlAttribute(episode.title)}', ${episode.unlock_cost_pkr || 0}, '${escapeJsStringForHtmlAttribute(episode.unlock_link || '')}')`;
            }

            buttonsHtml += `
                <button class="${buttonClass} font-bold py-2 px-4 rounded-lg transition-colors flex items-center justify-center text-sm" onclick="${onclickAction}">
                    EP ${episode.order || ('N/A')} ${buttonIcon}
                </button>
            `;
        }
    ...
    /**
     * NEW: Plays a selected episode in the modal.
     * @param {string} episodeId - The ID of the episode.
     * @param {string} videoUrl - The URL of the video for the episode.
     */
    function playEpisode(episodeId, videoUrl) {
        const playerContainer = document.getElementById('episode-video-player-container');
        if (playerContainer) {
            const embedUrl = convertVideoUrlToEmbed(videoUrl);

            // Handle non-embeddable external links by opening them directly
            if (embedUrl === videoUrl && !embedUrl.includes('youtube.com/embed') && !embedUrl.includes('drive.google.com/file')) {
                window.open(videoUrl, '_blank');
                showToast("Opening external link in new tab...");
                return;
            }
    ...
    /**
     * NEW: Prompts the user to unlock an episode.
     * @param {string} episodeId - The ID of the episode to unlock.
     * @param {string} episodeTitle - The title of the episode.
     * @param {number} cost - The cost to unlock the episode.
     * @param {string} unlockLink - An optional external link for unlocking.
     */
    async function unlockEpisodePrompt(episodeId, episodeTitle, cost, unlockLink) {
        if (!auth.currentUser) {
            showToast('Login required to unlock episodes!', true);
            return;
        }
        if (!currentUserData || currentUserData.locked) { 
            auth.signOut();
        ...
        const options = [];
        if (cost > 0) {
            options.push({ type: 'wallet', text: `Spend ${formatCurrency(cost, 'PKR')} from your wallet` });
        }
        if (unlockLink) {
            options.push({ type: 'link', text: `Visit an external link` });
        }

        if (options.length === 0) {
            return showToast(`This episode is locked and has no unlock options.`, true);
        }
        
        let promptMessage = `Unlock "${episodeTitle}" for 24 hours?\n\nChoose an option:\n`;
        ...
        if (!choice) return showToast('Unlock cancelled.', false);

        const chosenIndex = parseInt(choice) - 1;
        if (isNaN(chosenIndex) || chosenIndex < 0 || chosenIndex >= options.length) {
            return showToast('Invalid option selected.', true);
        }

        const chosenOption = options[chosenIndex];

        if (chosenOption.type === 'wallet') {
            if ((currentUserData.wallet.PKR || 0) < cost) {
                return showToast(`Insufficient PKR balance. You need ${formatCurrency(cost, 'PKR')} to unlock.`, true);
            }
            if (!confirm(`Confirm spending ${formatCurrency(cost, 'PKR')} to unlock "${episodeTitle}" for 24 hours?`)) {
                return showToast('Unlock cancelled.', false);
            }
            await deductAndUnlockEpisode(episodeId, episodeTitle, cost);
        } else if (chosenOption.type === 'link') {
            await openUnlockLink(episodeId, episodeTitle, unlockLink);
        }
    }
    ...
    async function renderVideoPlayerPage(container, data) {
        ...
        if (videoSourceUrl.includes('youtube.com/embed/') || videoSourceUrl.includes('drive.google.com/file/d/')) {
            ...
            // For other links, we assume they are embeddable. If not, the iframe might be blank.
            videoPlayerHtml = `<iframe src="${videoSourceUrl}" class="w-full h-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
        }
    ...
    function startVideoRewardTimer(totalSeconds, rewardPkr, videoId, videoTitle, sectionTitle) {
        ...
        videoRewardTimer = setInterval(async () => {
            ...
            if (timeLeft <= 0) {
                ...
                statusContainer.innerHTML = `<p class="font-bold text-yellow-400 mb-3 text-lg"><i class="fas fa-spinner fa-spin mr-2"></i>Claiming Reward...</p>`;
